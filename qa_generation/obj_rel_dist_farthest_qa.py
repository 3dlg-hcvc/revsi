...
QUESTION_TYPE = "object_rel_distance_farthest"
OBJ_REL_DISTANCE_QUESTION_TEMPLATE = """
Measuring from the closest point of each object, which of these objects ({choice_a}, {choice_b}, {choice_c}, {choice_d}) is the farthest from the {object_name}?
""".strip()
...
def _process_one_scene(indexed_scene_metadata, max_samples_per_scene, seed):
    ...
    for scene_obj in scene["objects"]:
        ...
        obj_name_to_geometry[obj_name] = (surface_points, obj_obb)
        obj_name_to_ids[obj_name].append(scene_obj["id"])
        single_instance_obj_names.append(obj_name)
    if len(single_instance_obj_names) < 5:
        return []

    obj_pair_distance_by_name = {}
    for pair in itertools.combinations(single_instance_obj_names, 2):
        first_surface_points, first_obb = obj_name_to_geometry[pair[0]]
        second_surface_points, second_obb = obj_name_to_geometry[pair[1]]
        ...
        obj_pair_distance_by_name[pair] = nearest_distance
        obj_pair_distance_by_name[(pair[1], pair[0])] = nearest_distance
    ...
    for primary_obj_name in single_instance_obj_names:
        ...
        option_obj_candidates = [obj_name for obj_name in single_instance_obj_names if obj_name != primary_obj_name]
        ...
        rows.append(
            {
                ...
                "gt": OPTION_LABEL_BY_INDEX[np.argmax(option_dists_to_primary)]
            }
        )
    return rows
...
if __name__ == '__main__':
    ...
    parser.add_argument("--max_samples_per_scene", "--max_sample_per_scene", type=int, default=7, help="Maximum number of samples per scene.")
    parser.add_argument("--seed", type=int, default=777, help="Random seed for reproducibility.")
    parser.add_argument("--output_path", type=str, default="obj_rel_dist_farthest.csv", help="Path to output preprocessed data.")
